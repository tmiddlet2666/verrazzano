...
if [ -z "${ADMIN_KUBECONFIG_ORIGINAL}" ] ; then
    echo "ADMIN_KUBECONFIG_ORIGINAL env var must be set!'"
    exit 1
fi
...
if [ -z "${MANAGED_KUBECONFIG_ORIGINAL}" ] ; then
    echo "MANAGED_KUBECONFIG_ORIGINAL env var must be set!'"
    exit 1
fi
...
echo ADMIN_KUBECONFIG_ORIGINAL: ${ADMIN_KUBECONFIG_ORIGINAL}
echo MANAGED_CLUSTER_NAME: ${MANAGED_CLUSTER_NAME}
echo MANAGED_KUBECONFIG: ${MANAGED_KUBECONFIG}
echo MANAGED_KUBECONFIG_ORIGINAL: ${MANAGED_KUBECONFIG_ORIGINAL}
...
if ! kubectl --kubeconfig ${ADMIN_KUBECONFIG} -n verrazzano-mc get configmap verrazzano-admin-cluster ; then
  export ADMIN_K8S_SERVER_ADDRESS=$(cat ${ADMIN_KUBECONFIG_ORIGINAL} | grep "server:" | awk '{ print $2 }')
...
echo "----------BEGIN ${MANAGED_CLUSTER_DIR}/managed_kube_config contents----------"
cat ${MANAGED_CLUSTER_DIR}/managed_kube_config
echo "----------END ${MANAGED_CLUSTER_DIR}/managed_kube_config contents----------"
