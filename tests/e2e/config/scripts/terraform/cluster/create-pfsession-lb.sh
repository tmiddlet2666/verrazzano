...
private_ip=$(kubectl --kubeconfig $KUBECONFIG get vz -o yaml | yq e ".items[0].status.instance.elasticUrl" - | cut -d "." -f5- | cut -d "n" -f1 | rev | cut -c2- | rev)
...
while IFS= read -r line; do
    sudo echo "$private_ip $(echo $line | cut -d "/" -f3-)" >> /etc/hosts
done <<< "$list"
sudo cat /etc/hosts
