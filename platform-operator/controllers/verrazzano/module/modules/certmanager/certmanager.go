...
func (r *Controller) applyManifest(compContext spi.ComponentContext) error {
...
func isCertManagerReady(context spi.ComponentContext) bool {
	namespace := context.Module().ChartNamespace()
...
func isCA(compContext spi.ComponentContext) (bool, error) {
	return validateConfiguration(compContext.EffectiveCR(), compContext.Module())
}
...
func validateConfiguration(cr *vzapi.Verrazzano, module *modulesv1alpha1.Module) (isCA bool, err error) {
	...
	if caNotEmpty {
		if err := validateCAConfiguration(components.CertManager.Certificate.CA, module); err != nil {
			return true, err
		}
		return true, nil
...
func validateCAConfiguration(ca vzapi.CA, module *modulesv1alpha1.Module) error {
	if ca.SecretName == constants.DefaultVerrazzanoCASecretName && ca.ClusterResourceNamespace == module.ChartNamespace() {
...
func createACMEIssuerObject(compContext spi.ComponentContext) (*unstructured.Unstructured, error) {
	...
	secret := v1.Secret{}
	namespace := compContext.Module().ChartNamespace()
	if err := compContext.Client().Get(context.TODO(), crtclient.ObjectKey{Name: ociDNSConfigSecret, Namespace: namespace}, &secret); err != nil {
...
func cleanupUnusedResources(compContext spi.ComponentContext, isCAValue bool) error {
	...
	if isCAValue {
		...
		if err := deleteObject(client, caAcmeSecretName, namespace, &v1.Secret{}); err != nil {
			return err
		}
	}
	if defaultCANotUsed() {
		...
		if err := deleteObject(client, caSelfSignedIssuerName, namespace, &certv1.Issuer{}); err != nil {
			return err
		}
		if err := deleteObject(client, caCertificateName, namespace, &certv1.Certificate{}); err != nil {
			return err
		}
		if err := deleteObject(client, defaultCACertificateSecretName, namespace, &v1.Secret{}); err != nil {
			return err
		}
	}
	return nil
