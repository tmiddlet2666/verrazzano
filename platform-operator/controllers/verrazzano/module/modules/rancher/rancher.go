...
func getRancherHostname(c clipkg.Client, vz *vzapi.Verrazzano) (string, error) {
...
func isRancherReady(ctx spi.ComponentContext) bool {
	log := ctx.Log()
	c := ctx.Client()
	namespace := ctx.Module().ChartNamespace()

	// Temporary work around for Rancher issue 36914
	err := checkRancherUpgradeFailure(c, log, namespace)
	if err != nil {
...
func checkRancherUpgradeFailure(client clipkg.Client, log vzlog.VerrazzanoLogger, namespace string) error {
	ctx := context.TODO()

	// Get the Rancher pods
	podList := &corev1.PodList{}
	err := client.List(ctx, podList, clipkg.InNamespace(namespace), clipkg.MatchingLabels{"app": "rancher"})
	if err != nil {
		return err
	}
	...
	for i, pod := range podList.Items {
		...
		logStream, err := clientSet.CoreV1().Pods(namespace).GetLogs(pod.Name, &corev1.PodLogOptions{Container: "rancher"}).Stream(ctx)
		if err != nil {
			return err
		}
		...
		if restartPod {
			...
			err = client.Delete(ctx, &podList.Items[i])
			if err != nil {
				return err
			}
			podsRestarted = true
